...
public class BinarySearchTree<ContentType extends ComparableContent<ContentType>> {
		...
		if (pContent.isLess(node.content)) {
			...
			if (node.left.isEmpty()) {
				if (node.right.isEmpty()) {
					...
					node = gibRechtenNachfolger();
				}
			} else if (node.right.isEmpty()) {
				// Es gibt nur links einen Nachfolger.
				node = gibLinkenNachfolger();
			} else {
				// Es gibt links und rechts einen Nachfolger.
				if (gibRechtenNachfolger().left.isEmpty()) {
					...
					BinarySearchTree<ContentType> vorgänger = node.right.ancestorOfSmallRight();
					BinarySearchTree<ContentType> smallest = vorgänger.node.left;
					this.node.content = smallest.node.content;
					vorgänger.remove(smallest.node.content);
				}
			}
		}
	}
	...
	private BSTNode<ContentType> gibLinkenNachfolger() {
		return node.left.node;
	}

	private BSTNode<ContentType> gibRechtenNachfolger() {
